...
def createAttentionRnn(attentionInputs,s0,c0,postAttentionInputs,n_a,n_s):
    s = s0
    c = c0
    # Defined shared layers as global variables
    biLstm=Bidirectional(LSTM(n_a,return_sequences=True))
...
def getTrainedattentionRnnModel(biLstm,repeator,concatenator,densor1,densor2,activator,dotor,s0,c0,concatenatorPost,post_activation_LSTM_cell,output_layer,n_a):
    ...
    aPostInput = Input(shape=(Constants.CNN_FINAL_H*Constants.CNN_FINAL_W,2*n_a))
    context = one_step_attention(aPostInput, s0,repeator,concatenator,densor1,densor2,activator,dotor)
