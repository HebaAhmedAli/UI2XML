# TODO: Create the cnn layers.
# TODO: Create the cnn model.
...
def lamdbda_split(value):
    ...
    x2 = tf.split(x, num_or_size_splits=x.shape[2], axis=1)  #(batch_size,1,1,depth)
...
def createCNN(xInput):
    ...
    x = Activation('relu')(x)
    x = Conv2D(256, (3, 3), padding='valid', activation='relu',data_format="channels_last")
    x = MaxPooling2D(pool_size=(2, 1))(x)
    x = Conv2D(512, (3, 3), padding='valid',data_format="channels_last")
    x = BatchNormalization(axis = 3)(x)
    x = Activation('relu')(x)
