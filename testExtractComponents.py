...
def processSave(subdir, file):
    ...
    XmlGeneration.generateXml(boxesFiltered,textsFiltered,predictedComponentsFiltered,imgCopy,file[:-4])
    for x,y,w,h in boxes:
        ...
        #cv2.imwrite(subdir + "/compOutputsAll"+file[:-4]+'/'+str(j) + str(file[len(file)-4:len(file)]),crop_img)
